...
export const Home = () => {
    ...
    useEffect(()=>{
        axios.get("http://localhost:3000/get").then(result =>setTodos(result.data)).catch(error => console.log("Use effect Error",error))
    }, [])

    const handleEdit =(id)=>{
        axios.put("http://localhost:3000/update/"+id).then(result =>setTodos(todos.map(todo =>
